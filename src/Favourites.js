		...
		<div className="blockquote text-center">
			...
			<div className="favourite-jokes">
				{favouriteJokes.array.map((item, index) =>
					<Joke key={item.id} joke={item} updateState={updateState} number={index+1}>
					</Joke>)}
			</div>
			<div className="clear-favourites">
				{!cleared ? <button className="btn btn-danger" onClick={clearFavourites}>Clear Favourites</button> : <span className="no-favorite">No Favourite Jokes</span> }
			</div>
		</div>
	)
}